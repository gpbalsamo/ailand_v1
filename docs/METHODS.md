...
|---|---|---|---|
| 0 | v0 as published | −1.325 | `swvl3` −1.235, `sro` −14.4 |
| 1 | Add snow prognostics `sd`, `rsn` | −1.197 | `snowc` 0.551 → **−1.490** (worse) |
| 2 | Drop runoff from the prognostic state (= v1's state vector) | **+0.723** | `swvl2` −0.545 → **0.741** |
| 3 | Add temporal/astronomical forcing | 0.747 | `swvl3` 0.668 → **0.956** |
| 4 | Scale the increments by tendency scalers | 0.763 | `swvl2` 0.741 → 0.871 |
| 5 | Both 3 and 4 | 0.746 | `swvl3` → **0.967** |
| 6 | Recover runoff as a *diagnostic* | — | `sro` −14.4 → **+0.234** |
| 7 | Replace XGBoost with the MLP + rollout loss | — | `snowc` 0.416 → **0.860**, `stl1` → 0.986 |

The two results that matter physically:

**Runoff must not be a prognostic variable.** Qs and Qsb (`sro`, `ssro`) are fluxes
*generated by* the soil column, not states *of* it. v0 carried them in the state vector
and fed them back every step. Their own rollout is meaningless (R² −14.4), and because
they also sit in the input vector, that noise propagated into the soil moisture. Moving
them to the diagnostic branch takes `sro` from −14.4 to +0.234 **and** takes `swvl2`
from −0.545 to 0.741. This is the single largest change in the whole exercise, and it
is a modelling error, not a tuning one.
