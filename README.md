...
### 1. A variable in the wrong place costs more than any amount of tuning

v0 carried surface and subsurface runoff as **prognostic** state, fed back into the
input vector every step. Runoff is a flux *generated by* the soil column, not a state
*of* it. Its own rollout was worthless (R² −14.4) and, because it also sat in the input
vector, that noise propagated into the soil moisture. Moving it to the diagnostic branch
took `sro` from −14.4 to +0.23 **and** `swvl2` from −0.55 to +0.74. One modelling
decision, worth more than every hyperparameter change combined.

### 2. Physically obvious is not empirically right
...
### 3. Read the error signature, not just the error
...
### 4. Silent NaN propagation deletes whole variables
...
### 5. Scale the targets, or the loss is only about the biggest number
...
### 6. Judge by metric, never by figure

v0's headline figure looks convincing because its y-range is dominated by the seasonal
cycle. Scored properly, v0 has a mean held-out R² of **−1.325**, with negative skill on
two of three soil moisture layers. The reported in-sample R² of 0.99 was `eval_set` set
to the training data.

### 7. Compare like with like before concluding anything is wrong
...
### 8. Small silent bugs in your own tooling
